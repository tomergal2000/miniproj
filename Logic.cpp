...
std::pair<glm::vec3, glm::vec2> calculateCameraState() {
    ...
    std::vector<cv::Point3f> objectPoints = convert3dPointToCV(mouseInputs.right);
    ...
    cv::Mat R;
    cv::Rodrigues(rvec, R); // efficient algorithm for rotating a vector in space, given an axis and angle of rotation
    ...
    glm::vec3 front = glm::normalize(glm::vec3(rotationMatrix[2]));
    //front = glm::normalize(front);

    float yaw1 = glm::degrees(atan2(front.z, front.x));
    float pitch1 = glm::degrees(asin(front.y));

    glm::vec3 upVector(R.at<double>(2, 0), R.at<double>(2, 1), R.at<double>(2, 2));
    upVectorPnP = upVector;

    return { cameraPosition, glm::vec2(yaw1, pitch1) };
    //return { translation, glm::vec2(yaw1, pitch1) };
}
